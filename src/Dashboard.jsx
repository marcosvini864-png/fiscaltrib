...
function TabBar({ tabs, activeTab, onTab }) {
  ...
  return (
    <div style={{ display:'flex', borderBottom:`2px solid ${C.border}`, background:C.white, padding:'0 24px', flexShrink:0 }}>
      {tabs.map((tab, i) => (
        <button key={i} onClick={() => onTab(i)}
          style={{ padding:'10px 18px', fontSize:13, fontWeight: activeTab===i ? 600 : 400, color: activeTab===i ? C.navy : C.muted, background:'none', border:'none', borderBottom: activeTab===i ? `2px solid ${C.navy}` : '2px solid transparent', marginBottom:-2, cursor:'pointer', whiteSpace:'nowrap', transition:'all 0.15s' }}
...
function Sidebar({ module, onNavigate, clientes, activeId, onChangeCliente, isAdmin }) {
  return (
  ...
  return (
    <div style={{maxWidth:860,margin:'0 auto'}}>
      <div style={{background:'linear-gradient(135deg,#0B1F4D,#163B8C)',borderRadius:16,padding:'28px 32px',color:'#fff',marginBottom:20}}>
        ...
        <h1 style={{fontSize:24,fontWeight:900,marginBottom:8,color:'#fff'}}>⚠️ Reforma Tributária</h1>
        <p style={{fontSize:14,color:'#93c5fd',margin:0}}>Consulte leis, decretos e impactos da Reforma — CBS, IBS, IS e período de transição (2026–2032).</p>
      </div>
      <div style={{background:'#fff',borderRadius:14,border:'2px solid #e2e8f0',padding:'20px 24px',marginBottom:16}}>
        ...
        <div style={{display:'flex',gap:10,marginBottom:14}}>
          <input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==='Enter'&&buscar()}
            ...
            style={{flex:1,padding:'10px 14px',border:'2px solid #e2e8f0',borderRadius:10,fontSize:13,color:C.text,outline:'none'}} />
          <button onClick={()=>buscar()} disabled={carregando}
            style={{padding:'10px 20px',background:C.navy,color:'#fff',border:'none',borderRadius:10,fontSize:13,fontWeight:700,cursor:carregando?'default':'pointer',opacity:carregando?0.7:1,whiteSpace:'nowrap'}}>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
          {SUGESTOES.map((s,i)=>(
            <button key={i} onClick={()=>{setQuery(s);buscar(s)}}
              style={{padding:'5px 10px',background:'#f1f5f9',border:'1px solid #e2e8f0',borderRadius:20,fontSize:11,color:'#475569',cursor:'pointer',fontWeight:500}}
              onMouseEnter={e=>e.currentTarget.style.background='#e2e8f0'}
              onMouseLeave={e=>e.currentTarget.style.background='#f1f5f9'}>{s}</button>
          ))}
        </div>
        {carregando&&<div style={{marginTop:16,background:'#f8fafc',borderRadius:10,padding:'16px 20px',display:'flex',alignItems:'center',gap:12}}><div style={{fontSize:18}}>⏳</div><div style={{fontSize:13,color:C.muted}}>Consultando base de conhecimento tributário...</div></div>}
        {resposta&&!carregando&&<div style={{marginTop:16,background:'#f0fdf4',border:'2px solid #86efac',borderRadius:10,padding:'16px 20px'}}><div style={{fontSize:11,fontWeight:700,color:'#16a34a',marginBottom:8,textTransform:'uppercase',letterSpacing:1}}>✅ Resposta da IA Tributária</div><div style={{fontSize:13,color:C.text,lineHeight:1.8,whiteSpace:'pre-wrap'}}>{resposta}</div></div>}
      </div>
      {historico.length>0&&<div style={{background:'#fff',borderRadius:14,border:'2px solid #e2e8f0',padding:'16px 24px',marginBottom:16}}>
        <div style={{fontSize:14,fontWeight:700,color:C.navy,marginBottom:12}}>🕓 Consultas anteriores</div>
        {historico.map((h,i)=>(
          <div key={i} style={{borderBottom:i<historico.length-1?'1px solid #f1f5f9':'none',paddingBottom:10,marginBottom:10}}>
            <div style={{fontSize:12,fontWeight:600,color:C.navy,marginBottom:3}}>❓ {h.pergunta}</div>
            <div style={{fontSize:11,color:C.muted,lineHeight:1.6,display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical',overflow:'hidden'}}>{h.resposta}</div>
            <button onClick={()=>{setQuery(h.pergunta);setResposta(h.resposta)}} style={{marginTop:4,fontSize:11,color:'#2563eb',background:'none',border:'none',cursor:'pointer',padding:0,fontWeight:600}}>Ver resposta completa →</button>
          </div>
        ))}
      </div>}
    </div>
          ...
          <div style={{flex:1,overflowY:'auto',overflowX:'hidden',padding:'24px 28px',background:C.bg,minWidth:0}}>
              ...
              {clientes.length===0 ? (
                ...
                <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,overflow:'hidden'}}>
                  <div style={{padding:'14px 18px',borderBottom:`1px solid ${C.border}`,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                    ...
                    <button onClick={()=>navigateTo('clientes',0)} style={{...btnOutline,padding:'5px 14px',fontSize:12}}>Ver todos</button>
                  </div>
                  <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
                    <thead><tr style={{background:'#F8FAFC'}}>{['Razão Social','CNPJ','Regime','Oportunidades','Potencial',''].map(h=><th key={h} style={{padding:'10px 16px',textAlign:'left',fontSize:11,fontWeight:600,color:C.muted,borderBottom:`1px solid ${C.border}`,textTransform:'uppercase',letterSpacing:0.5}}>{h}</th>)}</tr></thead>
                    <tbody>{clientes.map(c=>{const ee=entradas[c.id]||[];const tot=ee.reduce((s,e)=>s+(e.credito||0),0);return(
                      <tr key={c.id} style={{borderBottom:`1px solid ${C.border}`}}
                        ...
                        <td style={{padding:'12px 16px'}}><button onClick={()=>{setActiveId(c.id.toString());navigateTo('analise',0)}} style={{...btnOutline,padding:'4px 12px',fontSize:12}}>Analisar</button></td>
                      </tr>
                    )})}</tbody>
                  </table>
                </div>
            ...
            {module==='clientes' && activeTab===0 && <>
              <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:24}}>
                ...
                <button onClick={()=>handleTab(1)} style={{...btnPrimary,padding:'7px 16px',fontSize:13}}>+ Novo cliente</button>
              </div>
                  ...
                  <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between'}}>
                    ...
                    <div style={{textAlign:'right'}}>
                      ...
                      <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
                        <button onClick={()=>{setNovoCliente({...c});setActiveTab(1)}} style={{...btnOutline,padding:'4px 12px',fontSize:12}}>Editar</button>
                        <button onClick={()=>{setActiveId(c.id.toString());navigateTo('analise',0)}} style={{...btnPrimary,padding:'4px 12px',fontSize:12}}>Analisar</button>
            ...
            {module==='clientes' && activeTab===1 && novoCliente && <>
              ...
              {!novoCliente.id && (
                <div style={{background:'#eff6ff',border:'2px dashed #bfdbfe',borderRadius:12,padding:'18px 22px',marginBottom:20,display:'flex',alignItems:'center',justifyContent:'space-between',gap:16}}>
                  <div>
                    <div style={{fontSize:14,fontWeight:700,color:'#1e40af',marginBottom:4}}>⚡ Preencher via XML de NF-e</div>
                    <div style={{fontSize:13,color:C.muted}}>Importe um XML do cliente para preencher os dados automaticamente</div>
                  </div>
