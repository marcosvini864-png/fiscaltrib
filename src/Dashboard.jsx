...
// Teses do diagnóstico tributário — exibidas na segunda linha do topo
const TESES_DIAGNOSTICO = [
  { id: 'importar',   label: 'Importar',              icon: '📥' },
  { id: 'monofasicos', label: 'Monofásicos PIS/COFINS', icon: '💊' },
  { id: 'icms_tema69', label: 'ICMS Tema 69',           icon: '⚖️' },
  { id: 'icms_st',     label: 'ICMS-ST',                icon: '🔄' },
  { id: 'retencoes',   label: 'Retenções Indevidas',    icon: '🚫' },
  { id: 'pgdas',       label: 'PGDAS-D',                icon: '📋' },
]
  ...
  return (
    <div style={{maxWidth:860,margin:'0 auto'}}>
      <div style={{background:'linear-gradient(135deg,#0B1F4D,#163B8C)',borderRadius:16,padding:'24px',color:'#fff',marginBottom:20}}>
        ...
        <h1 style={{fontSize:20,fontWeight:900,marginBottom:8,color:'#fff'}}>Reforma Tributária</h1>
      ...
      <div style={{background:'#fff',borderRadius:14,border:'2px solid #e2e8f0',padding:'16px',marginBottom:16}}>
        ...
        <div style={{display:'flex',gap:8,marginBottom:14,flexWrap:'wrap'}}>
          <input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==='Enter'&&buscar()} placeholder="Ex: Como funciona a CBS?"
        ...
        {carregando&&<div style={{marginTop:16,background:'#f8fafc',borderRadius:10,padding:'16px',fontSize:13,color:C.muted}}>Consultando...</div>}
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100vh',width:'100vw',overflow:'hidden',fontFamily:'Inter,system-ui,sans-serif'}}>
      ...
      <div style={{background:C.navy, flexShrink:0}}>
        ...
        {/* Linha 1 — logo, cliente, usuário, sair */}
          ...
          <div style={{flex:1,overflowY:'auto',overflowX:'hidden',padding,background:C.bg,minWidth:0}}>
            ...
            {module==='painel' && <>
              ...
              <div style={{background:'#FFFBEB',border:'1px solid #FCD34D',borderRadius:8,padding:'10px 14px',marginBottom:16,fontSize:12,color:'#92400E'}}>
                <strong>Aviso:</strong> Análise preliminar — não dispensa revisão profissional.
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:12,marginBottom:20}}>
                <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,padding:'14px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
                  <div><div style={{fontSize:13,fontWeight:600,color:C.text,marginBottom:4}}>Prospeccao</div><div style={{fontSize:12,color:C.muted}}>Cockpit comercial e Kanban.</div></div>
                  <button onClick={()=>navigateTo('prospeccao')} style={{...btnOutline,padding:'6px 12px',fontSize:12,whiteSpace:'nowrap'}}>Abrir</button>
                </div>
                <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,padding:'14px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
                  <div><div style={{fontSize:13,fontWeight:600,color:C.text,marginBottom:4}}>Mensagens Rapidas</div><div style={{fontSize:12,color:C.muted}}>Templates para WhatsApp.</div></div>
            ...
            {/* ── DIAGNÓSTICO TRIBUTÁRIO com navegação por tese ── */}
            {module==='diagnostico' && (
            ...
            {module==='divida' && activeTab===0 && (
              <DiagnosticoDividaAtiva active={active} cdaParaDiagnostico={cdaParaDiagnostico} onCdaConsumed={() => setCdaParaDiagnostico(null)} />
            )}
            {module==='divida' && activeTab===1 && (
              <ImportarCDA active={active} onSalvo={()=>navigateTo('divida',0)}
                onDiagnostico={({campos,clienteEfetivo})=>{ setCdaParaDiagnostico({campos,clienteEfetivo}); navigateTo('divida',0) }}
