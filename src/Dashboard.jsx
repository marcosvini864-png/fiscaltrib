...
const CLIENTES_DEMO = [
  {id:'1',razao_social:'Farmácia Saúde Total Ltda',cnpj:'12.345.678/0001-90',municipio:'São Paulo',uf:'SP',cnae_principal:'4771-7/01',regime:'Simples Nacional',competencia_inicio:'2020-01',competencia_fim:'2024-12',responsavel_contabil:'Contabilidade ABC',observacoes:'Empresa vende produtos sujeitos à substituição tributária monofásica.',status:'Oportunidade encontrada'},
  {id:'2',razao_social:'Tech Soluções Digitais ME',cnpj:'98.765.432/0001-10',municipio:'Curitiba',uf:'PR',cnae_principal:'6201-5/01',regime:'Simples Nacional',competencia_inicio:'2019-01',competencia_fim:'2024-12',responsavel_contabil:'Escritório Contábil Sul',observacoes:'Serviços de TI — análise de Fator R.',status:'Oportunidade encontrada'},
  {id:'3',razao_social:'Construtora Horizonte Ltda',cnpj:'55.123.789/0001-44',municipio:'Belo Horizonte',uf:'MG',cnae_principal:'4120-4/00',regime:'Lucro Presumido',competencia_inicio:'2019-06',competencia_fim:'2024-12',responsavel_contabil:'Auditores MG',observacoes:'Construtora — análise PIS/COFINS.',status:'Oportunidade encontrada'},
  {id:'4',razao_social:'Assessoria Empresarial Delta Ltda',cnpj:'33.987.654/0001-22',municipio:'Rio de Janeiro',uf:'RJ',cnae_principal:'7020-4/00',regime:'Lucro Presumido',competencia_inicio:'2020-01',competencia_fim:'2024-12',responsavel_contabil:'Contabilidade Carioca',observacoes:'Verificar IRPJ/CSLL.',status:'Oportunidade encontrada'},
  {id:'5',razao_social:'Indústria Metalúrgica Norte S/A',cnpj:'11.222.333/0001-55',municipio:'Manaus',uf:'AM',cnae_principal:'2512-8/00',regime:'Lucro Real',competencia_inicio:'2018-01',competencia_fim:'2024-12',responsavel_contabil:'Auditores Norte',observacoes:'Zona Franca — créditos IPI e PIS/COFINS.',status:'Em análise'},
  {id:'6',razao_social:'Clínica Medicina Preventiva Ltda',cnpj:'44.555.666/0001-77',municipio:'Florianópolis',uf:'SC',cnae_principal:'8630-5/04',regime:'Lucro Presumido',competencia_inicio:'2021-01',competencia_fim:'2024-12',responsavel_contabil:'Contábil SC',observacoes:'Clínica médica — isenção IRPJ/CSLL.',status:'Em análise'},
]

const ENTRADAS_DEMO = {
  '1':[
    {id:'e1',competencia:'2022-01',tributo:'DAS',receita_bruta:80000,tributo_pago:3500,tributo_devido:1500,credito:2000,tipo_oportunidade:'Receita monofásica tributada indevidamente',risco:'baixo'},
    {id:'e2',competencia:'2022-02',tributo:'DAS',receita_bruta:82000,tributo_pago:3600,tributo_devido:1500,credito:2100,tipo_oportunidade:'Receita monofásica tributada indevidamente',risco:'baixo'},
    {id:'e3',competencia:'2022-03',tributo:'DAS',receita_bruta:85000,tributo_pago:3750,tributo_devido:1500,credito:2250,tipo_oportunidade:'Receita monofásica tributada indevidamente',risco:'baixo'},
  ],
  '2':[{id:'e4',competencia:'2021-01',tributo:'DAS',receita_bruta:45000,tributo_pago:2925,tributo_devido:1350,credito:1575,tipo_oportunidade:'Alíquota incorreta',risco:'medio'}],
  '3':[
    {id:'e5',competencia:'2020-06',tributo:'PIS/COFINS LP',receita_bruta:500000,tributo_pago:18000,tributo_devido:2500,credito:15500,tipo_oportunidade:'Base de cálculo reduzida não aplicada',risco:'baixo'},
    {id:'e6',competencia:'2020-07',tributo:'PIS/COFINS LP',receita_bruta:510000,tributo_pago:18360,tributo_devido:2600,credito:15760,tipo_oportunidade:'Base de cálculo reduzida não aplicada',risco:'baixo'},
  ],
  '4':[
    {id:'e7',competencia:'2021-03',tributo:'IRPJ/CSLL LP',receita_bruta:300000,tributo_pago:26400,tributo_devido:14400,credito:12000,tipo_oportunidade:'Base de cálculo reduzida não aplicada',risco:'medio'},
    {id:'e8',competencia:'2021-06',tributo:'IRPJ/CSLL LP',receita_bruta:320000,tributo_pago:28160,tributo_devido:16000,credito:12160,tipo_oportunidade:'Base de cálculo reduzida não aplicada',risco:'medio'},
    {id:'e9',competencia:'2021-09',tributo:'IRPJ/CSLL LP',receita_bruta:310000,tributo_pago:27280,tributo_devido:15200,credito:12080,tipo_oportunidade:'Base de cálculo reduzida não aplicada',risco:'medio'},
  ],
  '5':[],'6':[],
}
...
export default function Dashboard({ nomeUsuario, onLogout }) {
  ...
  function salvarCliente() {
    ...
    if(novoCliente.id) {
      setClientes(clientes.map(c=>c.id===novoCliente.id?novoCliente:c))
    } else {
      const nc = {...novoCliente, id: Date.now().toString(), status:'Em análise'}
      setClientes([...clientes,nc])
      setEntradas({...entradas,[nc.id]:[]})
      setActiveId(nc.id)
    }
    setPage('clientes')
  }
  ...
  function adicionarEntrada() {
    const e = {...novaEntrada, id:Date.now().toString()}
    e.credito = (parseFloat(e.tributo_pago)||0) - (parseFloat(e.tributo_devido)||0)
    if(e.credito<0) e.credito=0
    setEntradas({...entradas,[activeId]:[...(entradas[activeId]||[]),e]})
  ...
  const [cRbt12,setCRbt12]=useState(''); const [cRmes,setCRmes]=useState(''); const [cAnexo,setCAnexo]=useState('1')
...
<button onClick={()=>onLogout()} style={{background:'transparent',border:'1px solid #9db8d8',color:'#9db8d8',padding:'4px 12px',borderRadius:6,cursor:'pointer',fontSize:12}}>Sair</button>
     ...
     <div style={{display:'flex',flex:1,overflow:'hidden',width:'100%'}}>
        ...
        <div style={{width:220,background:'#fff',borderRight:'1px solid #e2e8f0',display:'flex',flexDirection:'column',flexShrink:0,overflowY:'auto'}}>
          ...
          <div style={{margin:12,padding:10,background:'#e0f2fe',borderRadius:8,fontSize:11,color:'#0369a1',lineHeight:1.5}}>
            <strong>6 casos de exemplo</strong> pré-carregados. Cadastre novos clientes ou edite os existentes.
          </div>
        </div>
        ...
        <div style={{flex:1,overflowY:'auto',overflowX:'hidden',padding:'28px 32px',background:'#f0f2f5',minWidth:0}}>
          ...
          {page==='novo-cliente' && novoCliente && <div>
            ...
            <div style={{display:'flex',gap:12}}>
              <button onClick={salvarCliente} style={{padding:'10px 20px',background:'#1e3a5f',color:'#fff',border:'none',borderRadius:6,fontSize:13,cursor:'pointer',fontWeight:500}}>💾 Salvar cliente</button>
          ...
          {page==='diagnostico' && <div>
            ...
            {ents.filter(e=>{const[a,m]=e.competencia.split('-');const l=new Date(parseInt(a)+5,parseInt(m)-1,1);return(l-hoje)/(1000*60*60*24*365)<=1&&e.credito>0}).length>0 && (
              <div style={{background:'#fff1f2',border:'1px solid #fca5a5',borderRadius:8,padding:'12px 16px',marginBottom:12,fontSize:13,color:'#991b1b'}}>
                🚨 <strong>ATENÇÃO — prazo prescricional:</strong> Há competências a menos de 1 ano do prazo de 5 anos para restituição/compensação.
              </div>
            )}
