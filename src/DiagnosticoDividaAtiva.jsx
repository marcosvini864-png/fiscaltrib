...
const CDA_VAZIA = {
  numero:'', tributo:'', valor:'', situacao:'Ativa',
...
function analisarDecadencia(cda) {
  ...
  if (!data_fato_gerador && !data_constituicao) {
    return { conclusao:'indefinida', titulo:'Decadência — Análise inconclusiva', cor:'#D97706', passos:[{ label:'Problema', valor:'Dados insuficientes', obs:'Não foi possível concluir porque não foram informadas a data do fato gerador nem a data da constituição definitiva do crédito.' }] }
  }
  const artigo = modalidade_lancamento==='homologacao' ? 'art. 150, §4º do CTN' : 'art. 173, I do CTN'
  const prazoAnos = 5
  const inicio = modalidade_lancamento==='homologacao' ? data_fato_gerador : data_fato_gerador
  const limite = addAnos(inicio, prazoAnos)
  const diasConst = diasEntre(inicio, data_constituicao)
  const prazoExcedido = data_constituicao && limite && data_constituicao > limite

  const passos = [
    { label:'Modalidade do lançamento', valor: modalidade_lancamento==='homologacao'?'Por homologação':'De ofício / Declaração', obs: modalidade_lancamento==='homologacao'?'Aplica-se o art. 150, §4º do CTN':'Aplica-se o art. 173, I do CTN' },
    { label:'Data do fato gerador', valor: fmtData(data_fato_gerador), obs:'Marco inicial para contagem do prazo decadencial' },
    { label:'Início da contagem', valor: fmtData(inicio), obs:`Regra aplicada: ${artigo}` },
    { label:'Prazo legal', valor:`${prazoAnos} anos`, obs:'Art. 173 / Art. 150, §4º do CTN' },
    ...
    { label:'Situação', valor: !data_constituicao ? 'Não verificável' : prazoExcedido ? '⚠️ CONSTITUIÇÃO FORA DO PRAZO' : '✅ Dentro do prazo', obs: prazoExcedido ? 'O crédito foi constituído após o término do prazo decadencial' : 'O crédito foi constituído dentro do prazo legal' },
  ]

  if (!data_constituicao) {
    return { conclusao:'indefinida', titulo:'Decadência — Análise inconclusiva', cor:'#D97706', passos, justificativa:'Não foi possível concluir a existência de decadência porque não foi informada a data da constituição definitiva do crédito.' }
  }

  if (prazoExcedido) {
    return {
      conclusao:'ha_decadencia', titulo:'⚠️ Há decadência', cor:'#DC2626', passos,
      justificativa:`O crédito tributário foi constituído em ${fmtData(data_constituicao)}, após o término do prazo decadencial de 5 anos previsto no ${artigo}, cujo limite era ${fmtData(limite)}. Não foi identificada qualquer causa legal que prorrogasse esse prazo.`
    }
  }

  return {
    conclusao:'sem_decadencia', titulo:'✅ Não há decadência', cor:'#16A34A', passos,
    justificativa:`O lançamento tributário foi constituído em ${fmtData(data_constituicao)}, dentro do prazo previsto no ${artigo}. Entre o fato gerador e a constituição definitiva do crédito não transcorreu prazo superior ao permitido pela legislação.`
  }
}
...
function analisarPrescricao(cda) {
  ...
  if (!data_constituicao) {
    return { conclusao:'indefinida', titulo:'Prescrição — Análise inconclusiva', cor:'#D97706', passos:[{ label:'Problema', valor:'Dados insuficientes', obs:'Não foi informada a data da constituição definitiva do crédito.' }] }
  }
  ...
  const passos = [
    { label:'Constituição definitiva do crédito', valor:fmtData(data_constituicao), obs:'Marco inicial da contagem prescricional (art. 174 CTN)' },
    { label:'Início da contagem', valor:fmtData(data_constituicao), obs:'Após a constituição definitiva' },
    { label:'Prazo legal', valor:'5 anos', obs:'Art. 174 do CTN' },
    { label:'Data-limite para cobrança', valor:fmtData(limite), obs:'Após essa data, sem interrupção, o crédito é prescrito' },
    { label:'Data do ajuizamento', valor:fmtData(data_ajuizamento)||'Não localizado', obs:interrompidoAjuizamento?'✅ Interrompe a prescrição (art. 174, pú., I)':'Não localizado dentro do prazo' },
    { label:'Data da citação válida', valor:fmtData(data_citacao)||'Não localizada', obs:interrompidoCitacao?'✅ Interrompe a prescrição (art. 174, pú., I)':'Não localizada' },
    { label:'Parcelamento', valor:possui_parcelamento?'Sim':'Não', obs:possui_parcelamento?'✅ Suspende a exigibilidade (art. 151, VI CTN)':'Não localizado' },
    { label:'Suspensão da exigibilidade', valor:possui_suspensao?'Sim':'Não', obs:possui_suspensao?'✅ Suspende o prazo prescricional':'Não localizada' },
    { label:'Situação atual', valor: prescrito?'⚠️ CRÉDITO PRESCRITO': suspenso||interrompidoCitacao||interrompidoAjuizamento?'✅ Prazo interrompido ou suspenso':'✅ Dentro do prazo', obs:'' },
  ]

  if (prescrito) {
    return {
      conclusao:'ha_prescricao', titulo:'⚠️ Há prescrição', cor:'#DC2626', passos,
      justificativa:`Transcorreram mais de cinco anos entre a constituição definitiva do crédito (${fmtData(data_constituicao)}) e a data atual, sem que fosse identificada citação válida, parcelamento ou qualquer outra causa legal de suspensão ou interrupção do prazo prescricional previsto no art. 174 do CTN. Data-limite: ${fmtData(limite)}.`
    }
  }

  if (interrompidoCitacao) {
    return {
      conclusao:'sem_prescricao', titulo:'✅ Não há prescrição', cor:'#16A34A', passos,
      justificativa:`O prazo prescricional foi interrompido pela citação válida do executado em ${fmtData(data_citacao)}, nos termos do art. 174, parágrafo único, inciso I do CTN, reiniciando sua contagem a partir dessa data.`
    }
  }
  ...
  if (suspenso) {
    return {
      conclusao:'sem_prescricao', titulo:'✅ Não há prescrição', cor:'#16A34A', passos,
      justificativa:`O prazo prescricional encontra-se suspenso em razão de ${possui_parcelamento?'parcelamento ativo (art. 151, VI do CTN)':'suspensão da exigibilidade identificada'}.`
    }
  }

  if (!data_citacao && !data_ajuizamento) {
    return {
      conclusao:'indefinida', titulo:'Prescrição — Análise inconclusiva', cor:'#D97706', passos,
      justificativa:`Não foi possível concluir a existência de prescrição porque não foi localizada a data da citação válida do executado nem do ajuizamento da execução fiscal.`
    }
  }

  return {
    conclusao:'sem_prescricao', titulo:'✅ Não há prescrição', cor:'#16A34A', passos,
    justificativa:`O crédito encontra-se dentro do prazo prescricional de 5 anos previsto no art. 174 do CTN. Data-limite: ${fmtData(limite)}.`
  }
}
...
function analisarPrescricaoIntercorrente(cda) {
  ...
  if (!data_ajuizamento) {
    return { conclusao:'indefinida', titulo:'Prescrição Intercorrente — Inconclusiva', cor:'#D97706', passos:[{ label:'Problema', valor:'Dados insuficientes', obs:'Não foi informada a data do ajuizamento da execução fiscal.' }] }
  }

  const referenciaContagem = data_ultima_movimentacao || data_ajuizamento
  const limite = addAnos(referenciaContagem, 5)
  const diasParado = diasEntre(referenciaContagem, hoje)
  const prescritoIntercorrente = !possui_embargos && !possui_penhora && hoje > limite
  ...
  const passos = [
    ...
    { label:'Data-limite (5 anos sem impulso)', valor:fmtData(limite), obs:'Após esse prazo sem movimentação útil, configura-se a prescrição intercorrente' },
    { label:'Penhora de bens', valor:possui_penhora?'Sim':'Não', obs:possui_penhora?'✅ Indica movimentação processual ativa':'Não localizada' },
    { label:'Embargos à execução', valor:possui_embargos?'Sim':'Não', obs:possui_embargos?'✅ Indica impulso processual':'Não localizados' },
    { label:'Situação', valor:prescritoIntercorrente?'⚠️ POSSÍVEL PRESCRIÇÃO INTERCORRENTE':'✅ Sem prescrição intercorrente identificada', obs:'' },
  ]

  if (prescritoIntercorrente) {
    return {
      conclusao:'ha_prescricao_intercorrente', titulo:'⚠️ Possível prescrição intercorrente', cor:'#DC2626', passos,
      justificativa:`Foi identificada possível prescrição intercorrente porque o processo permaneceu sem movimentação útil por período superior a 5 anos (desde ${fmtData(referenciaContagem)}), sem localização de bens ou atos processuais capazes de interromper o prazo, nos termos do art. 40 da Lei 6.830/80 e Súmula 314 do STJ.`
    }
  }

  return {
    conclusao:'sem_prescricao_intercorrente', titulo:'✅ Sem prescrição intercorrente', cor:'#16A34A', passos,
    justificativa:`Não foi identificada prescrição intercorrente porque ${possui_penhora||possui_embargos?'o processo apresenta movimentação processual ativa (penhora ou embargos identificados)':'o processo permaneceu dentro do prazo legal de 5 anos sem impulso processual'}.`
  }
}
...
function analisarCDA(cda) {
  ...
  else ok.push(`Número da CDA: ${cda.numero}`)

  if (!cda.tributo) problemas.push('Tributo não identificado')
  else ok.push(`Tributo identificado: ${cda.tributo}`)

  if (!cda.valor) problemas.push('Valor da CDA não informado — prejudica análise de liquidez e certeza')
  else ok.push(`Valor informado: ${cda.valor}`)

  if (!cda.data_constituicao) problemas.push('Data de constituição não informada — impede verificação de decadência')
  if (!cda.data_inscricao) problemas.push('Data de inscrição não informada')
  if (!cda.data_fato_gerador) problemas.push('Data do fato gerador não informada — impede aplicação das regras de decadência')

  const passos = [
    { label:'Número da CDA',              valor:cda.numero||'Não informado',         obs: cda.numero?'✅ Identificada':'⚠️ Ausente — requisito obrigatório (art. 202, I CTN)' },
    { label:'Tributo',                    valor:cda.tributo||'Não identificado',      obs: cda.tributo?'✅ Identificado':'⚠️ Ausente — requisito de certeza' },
    { label:'Valor',                      valor:cda.valor||'Não informado',           obs: cda.valor?'✅ Informado':'⚠️ Ausente — requisito de liquidez' },
    { label:'Data de constituição',       valor:fmtData(cda.data_constituicao),       obs: cda.data_constituicao?'✅ Informada':'⚠️ Ausente — impede análise de decadência' },
    { label:'Data de inscrição',          valor:fmtData(cda.data_inscricao),          obs: cda.data_inscricao?'✅ Informada':'⚠️ Ausente' },
    { label:'Liquidez e certeza',         valor:cda.valor&&cda.tributo?'✅ Presentes':'⚠️ Verificar', obs:'Requisitos essenciais da CDA (art. 202 CTN)' },
    { label:'Exigibilidade',              valor:cda.situacao,                         obs:`Situação atual: ${cda.situacao}` },
    { label:'Problemas identificados',    valor:`${problemas.length} item(ns)`,       obs: problemas.join('; ')||'Nenhum problema identificado' },
  ]

  if (problemas.length === 0) {
    return {
      conclusao:'cda_ok', titulo:'✅ CDA sem vícios aparentes', cor:'#16A34A', passos,
      justificativa:'A CDA apresenta todos os requisitos formais verificáveis com base nos dados informados, incluindo identificação do crédito, tributo, valor e datas essenciais.'
    }
  }

  return {
    conclusao:'cda_vicio', titulo:'⚠️ Possíveis vícios na CDA', cor:'#DC2626', passos,
    justificativa:`Foram identificados ${problemas.length} ponto(s) que merecem verificação: ${problemas.join('; ')}. Recomenda-se conferência da CDA original junto à PGFN.`
  }
}

function gerarParecer(analisesCDA) {
  const parecer = []
  let urgente = false
  ...
  analisesCDA.forEach((a, i) => {
    ...
    if (a.prescricaoIntercorrente.conclusao==='ha_prescricao_intercorrente') { parecer.push({ tipo:'danger', msg:`${num}: Prescrição intercorrente identificada — verificar imediatamente.` }); urgente=true }
    if (a.cda.conclusao==='cda_vicio') parecer.push({ tipo:'warning', msg:`${num}: Possíveis vícios formais na CDA identificados.` })
    if (a.decadencia.conclusao==='indefinida') parecer.push({ tipo:'warning', msg:`${num}: Decadência inconclusiva — complementar dados para análise completa.` })
    if (a.prescricao.conclusao==='indefinida') parecer.push({ tipo:'warning', msg:`${num}: Prescrição inconclusiva — informar data de citação e ajuizamento.` })
  })
...
function ResultadoAnalise({ resultado }) {
  ...
  return (
    <div style={{background: resultado.conclusao.includes('ha_')||resultado.conclusao==='cda_vicio'?'#FEF2F2':resultado.conclusao==='indefinida'?'#FFFBEB':'#F0FDF4', border:`1px solid ${resultado.cor}33`, borderLeft:`4px solid ${resultado.cor}`, borderRadius:10, padding:'14px 18px', marginBottom:10}}>
      ...
      {expandido && resultado.passos && (
        <div style={{marginTop:12,background:'rgba(255,255,255,0.7)',borderRadius:8,padding:'12px 16px'}}>
          <div style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:10,textTransform:'uppercase',letterSpacing:1}}>Raciocínio aplicado</div>
...
export default function DiagnosticoDividaAtiva({ active }) {
  ...
  const [dados, setDados] = useState({
    cnpj: active?.cnpj||'', valor_total:'', orgao_credor:'PGFN',
    processo_execucao:'', possui_parcelamento:false, possui_transacao_anterior:false,
    possui_garantia:false, possui_penhora:false, possui_bloqueio:false,
    possui_embargos:false, observacoes:'',
  })
  ...
  async function salvar() {
    ...
    try {
      const { data:{ user } } = await supabase.auth.getUser()
      const payload = {
        usuario_id:user.id, cliente_id:active?.id||null,
        razao_social:active?.razao_social||'', cnpj:dados.cnpj,
        valor_total:dados.valor_total, orgao_credor:dados.orgao_credor,
        ...
        possui_garantia:dados.possui_garantia, possui_penhora:dados.possui_penhora,
        possui_bloqueio:dados.possui_bloqueio, possui_embargos:dados.possui_embargos,
        observacoes:dados.observacoes, cdas:cdas,
        diagnostico:diagnostico, score:diagnostico?.score||null,
  ...
  async function duplicarRegistro(reg) {
    try {
      const { data:{ user } } = await supabase.auth.getUser()
      const { data } = await supabase.from('divida_ativa').insert([{...reg, id:undefined, usuario_id:user.id, razao_social:(reg.razao_social||'')+'  (cópia)', created_at:new Date().toISOString(), updated_at:new Date().toISOString()}]).select()
      if(data?.[0]) setHistorico(prev=>[data[0],...prev])
      alert('✅ Duplicado!')
    } catch(e){ alert('Erro: '+e.message) }
  }
  ...
  function novoRegistro() {
    ...
    setCdas([{...CDA_VAZIA}])
    setDiagnostico(null); setAnalisesCDA([])
  ...
  function executarDiagnostico() {
    ...
    setTimeout(()=>{
      ...
      setDiagnostico({ parecer, urgente, score, valor, data:new Date().toISOString() })
      setAnalisando(false)
      setAba(2)
    },1500)
  }
  ...
  function calcularSimulacao() {
    ...
    const multa=valor*(sim.multa_pct/100); const juros=valor*(sim.juros_pct/100)
  ...
  function gerarRelatorio() {
    ...
    const linhas=[
      '╔══════════════════════════════════════════════════════════════╗',
      '║      FISCALTRIB — PARECER TÉCNICO — DÍVIDA ATIVA (PGFN)     ║',
      '╚══════════════════════════════════════════════════════════════╝',
      '',`Cliente: ${active?.razao_social||dados.cnpj}`,`CNPJ: ${dados.cnpj}`,`Data: ${new Date().toLocaleDateString('pt-BR')}`,`Score: ${diagnostico.score}/100`,
      '','═══ PARECER FINAL ════════════════════════════════════════════',
      ...diagnostico.parecer.map(p=>`${p.tipo==='danger'?'⚠️':'ℹ️'} ${p.msg}`),
      '',
      ...analisesCDA.map((a,i)=>[
        `═══ CDA ${i+1}: ${a.cda.numero||'Sem número'} ════════════════════════════`,
        `Tributo: ${a.cda.tributo||'—'} | Valor: ${a.cda.valor||'—'} | Situação: ${a.cda.situacao}`,
        '',`── DECADÊNCIA: ${a.decadencia.titulo}`,a.decadencia.justificativa||'',
        '',`── PRESCRIÇÃO: ${a.prescricao.titulo}`,a.prescricao.justificativa||'',
        '',`── PRESCRIÇÃO INTERCORRENTE: ${a.prescricaoIntercorrente.titulo}`,a.prescricaoIntercorrente.justificativa||'',
        '',`── VALIDADE DA CDA: ${a.validadeCDA.titulo}`,a.validadeCDA.justificativa||'',
        '',`── RACIOCÍNIO — DECADÊNCIA:`,
        ...(a.decadencia.passos||[]).map(p=>`  ${p.label}: ${p.valor} — ${p.obs}`),
        '',`── RACIOCÍNIO — PRESCRIÇÃO:`,
        ...(a.prescricao.passos||[]).map(p=>`  ${p.label}: ${p.valor} — ${p.obs}`),
        '',
      ].join('\n')),
      '','Gerado por FiscalTrib — fiscaltrib.com.br',
      'Este parecer é preliminar e não substitui análise jurídica profissional.',
    ]
    const blob=new Blob([linhas.join('\n')],{type:'text/plain;charset=utf-8'})
    const url=URL.createObjectURL(blob)
    const a=document.createElement('a')
    a.href=url;a.download=`Parecer_DividaAtiva_${dados.cnpj||'cliente'}_${hoje}.txt`
  ...
  const inp = (k,ph,tp='text') => {
    const handleChange = e => {
      ...
      if(k==='processo_execucao'){
        ...
        v=v.replace(/^(\d{7})(\d)/,'$1-$2').replace(/^(\d{7})-(\d{2})(\d)/,'$1-$2.$3').replace(/^(\d{7})-(\d{2})\.(\d{4})(\d)/,'$1-$2.$3.$4').replace(/^(\d{7})-(\d{2})\.(\d{4})\.(\d{1})(\d)/,'$1-$2.$3.$4.$5')
      }
      setDados({...dados,[k]:v})
    }
    const handleBlur = e => {
      if(k==='valor_total'){ const raw=e.target.value.replace(/\./g,'').replace(',','.'); const n=parseFloat(raw)||0; if(n>0) setDados(d=>({...d,valor_total:n.toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})})) }
    }
  ...
  if(tela==='historico') return (
    <div style={{maxWidth:960,margin:'0 auto'}}>
      <div style={{background:'linear-gradient(135deg,#1e293b,#0B1F4D)',borderRadius:16,padding:'28px 32px',color:'#fff',marginBottom:20}}>
        ...
        <p style={{fontSize:14,color:'#cbd5e1',margin:0}}>Motor de inteligência jurídica · Análise automática de decadência, prescrição e validade da CDA</p>
      </div>
      ...
      {loadingHist ? <div style={{textAlign:'center',padding:40,color:C.muted}}>Carregando...</div>
      : historico.length===0 ? (
        ...
        <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,overflow:'hidden'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
            <thead><tr style={{background:'#F8FAFC'}}>{['Cliente','CNPJ','Valor Total','CDAs','Score','Data',''].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:11,fontWeight:600,color:C.muted,borderBottom:`1px solid ${C.border}`,textTransform:'uppercase',letterSpacing:0.5}}>{h}</th>)}</tr></thead>
            <tbody>
              {historico.map(reg=>(
                <tr key={reg.id} style={{borderBottom:`1px solid ${C.border}`}}
                  ...
                  <td style={{padding:'12px 14px'}}>
                    <div style={{display:'flex',gap:6}}>
                      ...
                      <button onClick={()=>duplicarRegistro(reg)} style={{...btnOutline,padding:'4px 10px',fontSize:12}}>📋</button>
    ...
    <div style={{maxWidth:960,margin:'0 auto'}}>
      ...
      {aba===0&&<>
        {!diagnostico?(
          <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,padding:'32px',textAlign:'center'}}>
            ...
            <div style={{fontSize:13,color:C.muted,marginBottom:20}}>Acesse "Dados da Dívida", preencha os dados e execute o diagnóstico inteligente.</div>
          ...
          {diagnostico.parecer.length===0&&(
            <div style={{background:'#F0FDF4',border:'1px solid #86EFAC',borderRadius:8,padding:'12px 16px',fontSize:13,color:'#166534',marginBottom:16}}>
              ✅ Nenhuma irregularidade grave identificada nas CDAs analisadas.
            </div>
          )}
      ...
      {aba===1&&<>
        <div style={{background:'#EFF6FF',border:'1px solid #BFDBFE',borderRadius:10,padding:'12px 16px',marginBottom:16,fontSize:12,color:'#1E40AF'}}>
          ℹ️ <strong>Modo manual.</strong> Preencha todos os campos possíveis para que o motor de inteligência realize a análise mais completa.
        </div>
          ...
          {cdas.map((cda,i)=>(
            <div key={i} style={{background:'#F8FAFC',borderRadius:10,border:`1px solid ${C.border}`,padding:'16px 20px',marginBottom:12}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginBottom:12}}>
                ...
                <div><label style={{fontSize:12,fontWeight:500,display:'block',marginBottom:4,color:C.text}}>Tributo</label><input value={cda.tributo} onChange={e=>updateCDA(i,'tributo',e.target.value)} placeholder="IRPJ, CSLL, PIS..." style={{padding:'6px 10px',border:`1px solid ${C.border}`,borderRadius:6,fontSize:12,width:'100%',boxSizing:'border-box'}}/></div>
      ...
      {aba===2&&<>
        {analisesCDA.length===0?(
          ...
          {analisesCDA.map((a,i)=>(
            <div key={i} style={{background:C.white,borderRadius:12,border:`1px solid ${C.border}`,padding:'20px 24px',marginBottom:16}}>
              <div style={{fontSize:15,fontWeight:700,color:C.navy,marginBottom:4}}>
                CDA {i+1}{a.cda.numero?` — ${a.cda.numero}`:''}
              </div>
              <div style={{fontSize:12,color:C.muted,marginBottom:16}}>
                {a.cda.tributo||'Tributo não informado'} · {a.cda.valor||'Valor não informado'} · {a.cda.situacao}
              </div>
          ...
          <div style={{background:'#F0FDF4',border:'1px solid #86EFAC',borderRadius:10,padding:'14px 18px',fontSize:12,color:'#166534',marginBottom:16}}>
            💡 Clique em "▼ Ver raciocínio" em cada análise para ver o passo a passo do raciocínio jurídico aplicado.
          </div>
          ...
          {!diagnostico||analisesCDA.length===0?(
            ...
            {analisesCDA.map((a,i)=>(
              <div key={i} style={{borderLeft:`4px solid ${C.navy}`,paddingLeft:16,marginBottom:20}}>
                <div style={{fontSize:14,fontWeight:700,color:C.navy,marginBottom:12}}>CDA {i+1}{a.cda.numero?` — ${a.cda.numero}`:''}</div>
                {[
                  ['Decadência',a.decadencia],
                  ['Prescrição',a.prescricao],
                  ['Prescrição Intercorrente',a.prescricaoIntercorrente],
                  ['Validade da CDA',a.validadeCDA],
                ].map(([titulo,res])=>(
            ...
            <div style={{background:'#FFFBEB',border:'1px solid #FCD34D',borderRadius:8,padding:'12px 16px',fontSize:12,color:'#92400E',marginBottom:16}}>
              ⚠️ Este parecer é preliminar e não substitui análise jurídica profissional. Recomenda-se validação por advogado tributarista habilitado antes de qualquer medida judicial ou administrativa.
            </div>
