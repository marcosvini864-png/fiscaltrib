...
function parseSPED(txtStr) {
  ...
  const r = { competencia:'', totalSaidas:0, totalICMS:0, totalPIS:0, totalCOFINS:0, linhas:linhas.length, valido:false }
...
function detectarRaioX(nfes, regime) {
  ...
  if (mono.length > 0) {
    const mm = mono.reduce((s,i)=>s+i.vItemPIS+i.vItemCOFINS,0) / meses || mono.reduce((s,i)=>s+i.vProd,0)/meses*0.0365
...
export default function EntradaDados({ clienteId, cliente, onSalvo, setPage }) {
  const [etapa,        setEtapa]        = useState('inicio') // inicio | importando | manual | concluido
  ...
  const nfeRef  = useRef(); const pgdasRef = useRef()
  const spedRef = useRef(); const outroRef = useRef()
  ...
  async function buscarCNPJ() {
    ...
    } catch { alert('Erro ao consultar. Verifique o CNPJ.') }
    setBuscando(false)
  }
  ...
  function processarNFe(files) {
    ...
    Array.from(files).forEach(f => {
      if (!f.name.endsWith('.xml')) { erros.push(f.name); pend--; return }
  ...
  function processarPGDAS(file) {
    ...
    r.onload = e => {
      ...
      setDadosImport({ tipo:'PGDAS-D', receita:d.receitaBruta, impostos:d.dasDevido, periodo:d.competencia, competencias:1, nfes:0, pgdas:d })
      setEtapa('concluido')
    }
  ...
  function processarSPED(file) {
    ...
    r.onload = e => {
      ...
      setDadosImport({ tipo:'SPED', receita:d.totalSaidas, impostos:d.totalICMS+d.totalPIS+d.totalCOFINS, periodo:d.competencia, competencias:1, nfes:0, sped:d })
      setEtapa('concluido')
    }
    r.readAsText(file,'UTF-8')
  }
  ...
  async function salvarManual() {
    ...
    const credito = Math.max(0, pago - devido)
    await supabase.from('entradas').insert([{
      ...
      tributo_pago: pago, tributo_devido: devido, credito,
  ...
  // ── Estilos ──────────────────────────────────────────────────────────────────
  const cardImport = (icon, label, desc, onClick, cor='#0B1F4D') => (
    <div onClick={onClick}
      style={{ background:'#fff', border:'2px solid #e2e8f0', borderRadius:14, padding:'20px', cursor:'pointer', textAlign:'center', transition:'all 0.15s' }}
      onMouseEnter={e=>{ e.currentTarget.style.borderColor=cor; e.currentTarget.style.background='#f8fafc'; e.currentTarget.style.transform='translateY(-2px)' }}
  ...
  return (
    <div style={{ maxWidth:900, margin:'0 auto' }}>
      ...
      {/* Header */}
      <div style={{ background:'linear-gradient(135deg, #0B1F4D 0%, #163B8C 100%)', borderRadius:16, padding:'24px 32px', marginBottom:24, color:'#fff' }}>
        <div style={{ fontSize:11, color:'#7CC4FF', fontWeight:700, letterSpacing:2, marginBottom:6 }}>FISCALTRIB — COLETA INTELIGENTE</div>
        <h2 style={{ fontSize:22, fontWeight:900, marginBottom:6 }}>📥 Centro de Coleta de Dados</h2>
        <p style={{ fontSize:13, color:'#93c5fd', margin:0 }}>
          Importe os arquivos fiscais — o sistema preenche tudo automaticamente.
        </p>
      </div>
      ...
      {!clienteId ? (
        ...
        <>
          ...
          <div style={{ background:'#fff', borderRadius:14, border:'2px solid #e2e8f0', padding:'20px 24px', marginBottom:16 }}>
            ...
            <div style={{ fontSize:12, color:'#64748b', marginBottom:16 }}>Busque pelo CNPJ para preenchimento automático ou use os dados do cliente selecionado</div>
            ...
            <div style={{ background:'#f0f9ff', border:'1.5px solid #bae6fd', borderRadius:10, padding:'14px 18px', marginBottom:14, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              ...
              <span style={{ background:'#0B1F4D', color:'#fff', fontSize:11, padding:'4px 12px', borderRadius:99, fontWeight:700 }}>✓ Selecionado</span>
            </div>

            {/* Busca CNPJ */}
            <div style={{ display:'flex', gap:10, alignItems:'center' }}>
              <input value={cnpjBusca} onChange={e=>setCnpjBusca(maskCNPJ(e.target.value))} placeholder="Buscar outro CNPJ..."
                style={{ flex:1, padding:'10px 14px', border:'2px solid #e2e8f0', borderRadius:8, fontSize:13, color:'#374151' }} />
              <button onClick={buscarCNPJ} disabled={buscando}
                style={{ padding:'10px 20px', background:'#0B1F4D', color:'#fff', border:'none', borderRadius:8, fontSize:13, fontWeight:700, cursor:'pointer', whiteSpace:'nowrap' }}>
                {buscando ? '⏳' : '🔍 Buscar'}
              </button>
            </div>
            {empresaBusca && (
          ...
          {/* ── BLOCO 2: Fontes de Dados ── */}
          <div style={{ background:'#fff', borderRadius:14, border:'2px solid #e2e8f0', padding:'20px 24px', marginBottom:16 }}>
            <div style={{ fontSize:13, fontWeight:800, color:'#0B1F4D', marginBottom:4 }}>📂 Bloco 2 — Fontes de dados</div>
            <div style={{ fontSize:12, color:'#64748b', marginBottom:16 }}>Clique no tipo de arquivo para importar — o sistema lê e preenche tudo automaticamente</div>

            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:16 }}>
              {cardImport('🧾','XML NF-e em lote','Múltiplos arquivos XML', () => nfeRef.current?.click(), '#16a34a')}
              {cardImport('📋','PGDAS-D','Arquivo XML do Simples Nacional', () => pgdasRef.current?.click(), '#2563eb')}
              {cardImport('📂','SPED Fiscal / Contribuições','Arquivo TXT do SPED', () => spedRef.current?.click(), '#7c3aed')}
              {cardImport('📑','DCTFWeb / Outros','Demais arquivos fiscais', () => setPage && setPage('importacoes'), '#d97706')}
            </div>
            ...
            {/* Botão entrada manual escondido */}
            {etapa !== 'manual' && (
              <div style={{ textAlign:'center' }}>
                <button onClick={()=>setEtapa('manual')}
          ...
          {/* ── BLOCO 4: Raio-X Instantâneo ── */}
          {raioX.length > 0 && (
            <div style={{ background:'#fff', borderRadius:14, border:'2px solid #e2e8f0', padding:'20px 24px', marginBottom:16 }}>
              <div style={{ fontSize:13, fontWeight:800, color:'#0B1F4D', marginBottom:4 }}>⚡ Bloco 4 — Raio-X Tributário</div>
              <div style={{ fontSize:12, color:'#64748b', marginBottom:16 }}>Oportunidades detectadas automaticamente nos arquivos importados</div>
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                ...
                <div style={{ background:'linear-gradient(135deg,#0B1F4D,#163B8C)', borderRadius:10, padding:'14px 20px', display:'flex', justifyContent:'space-between', alignItems:'center', color:'#fff' }}>
              ...
              <div style={{ display:'flex', gap:10, marginTop:16 }}>
                <button onClick={()=>setPage&&setPage('diagnostico')}
                  style={{ flex:1, padding:'12px 0', background:'#eff6ff', border:'2px solid #bfdbfe', color:'#1e40af', borderRadius:10, fontSize:13, fontWeight:700, cursor:'pointer' }}>
                  📋 Ver diagnóstico completo
                </button>
                <button onClick={()=>setPage&&setPage('relatorio')}
                  style={{ flex:1, padding:'12px 0', background:'#f0fdf4', border:'2px solid #86efac', color:'#166534', borderRadius:10, fontSize:13, fontWeight:700, cursor:'pointer' }}>
            ...
            <div style={{ background:'#fff', borderRadius:14, border:'2px solid #fde68a', padding:'20px 24px', marginBottom:16 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
                <div>
                  ...
                  <div style={{ fontSize:12, color:'#94a3b8' }}>Prefira sempre importar arquivos fiscais para maior precisão</div>
                </div>
