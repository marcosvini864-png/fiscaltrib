...
export default function EntradaDados({ clienteId, cliente, onSalvo, setPage }) {
  ...
  async function buscarCNPJ() {
    ...
    try {
      ...
      if (data.cnpj) {
        setEmpresaBusca({ razao_social: data.razao_social, cnpj: maskCNPJ(data.cnpj), cnae: data.cnae_fiscal, municipio: data.municipio, uf: data.uf, situacao: data.descricao_situacao_cadastral })
  ...
  function processarPGDAS(file) {
    ...
    r.onload = e => {
      ...
      setDadosImport({ tipo:'PGDAS-D', receita: d.receitaBruta, impostos: d.dasDevido, periodo: d.competencia, nfes: 0, pgdas: d })
      setEtapa('concluido')
    }
  ...
  function processarSPED(file) {
    ...
    r.onload = e => {
      ...
      setDadosImport({ tipo:'SPED', receita: d.totalSaidas, impostos: d.totalICMS + d.totalPIS + d.totalCOFINS, periodo: d.competencia, nfes: 0, sped: d })
      setEtapa('concluido')
    }
    r.readAsText(file, 'UTF-8')
  }
  ...
  const Card = ({ icon, label, desc, onClick, cor }) => (
    <div onClick={onClick} style={{ background:'#fff', border:'2px solid #e2e8f0', borderRadius:14, padding:20, cursor:'pointer', textAlign:'center', transition:'all 0.15s' }}
  ...
  return (
    <div style={{ maxWidth:900, margin:'0 auto' }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#0B1F4D,#163B8C)', borderRadius:16, padding:'24px 32px', marginBottom:24, color:'#fff' }}>
        ...
        <h2 style={{ fontSize:22, fontWeight:900, marginBottom:6, margin:0 }}>📥 Centro de Coleta de Dados</h2>
        <p style={{ fontSize:13, color:'#93c5fd', margin:'6px 0 0' }}>Importe os arquivos fiscais — o sistema preenche tudo automaticamente.</p>
      </div>
        ...
        <>
          ...
          <div style={{ background:'#fff', borderRadius:14, border:'2px solid #e2e8f0', padding:'20px 24px', marginBottom:16 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:14 }}>
              <Card icon="🧾" label="XML NF-e em lote"          desc="Múltiplos arquivos XML"         onClick={() => nfeRef.current?.click()}              cor="#16a34a" />
              <Card icon="📋" label="PGDAS-D"                   desc="XML do Simples Nacional"        onClick={() => pgdasRef.current?.click()}            cor="#2563eb" />
              <Card icon="📂" label="SPED Fiscal / Contribuições" desc="Arquivo TXT do SPED"          onClick={() => spedRef.current?.click()}             cor="#7c3aed" />
              <Card icon="📑" label="DCTFWeb / Outros"           desc="Demais arquivos fiscais"        onClick={() => setPage && setPage('importacoes')}    cor="#d97706" />
            </div>
