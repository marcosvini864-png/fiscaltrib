...
export default function Login({ onLogin, onCadastro }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#1e3a5f', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
      <div style={{ background:'#fff', borderRadius:16, padding:'24px 40px', width:400, boxShadow:'0 20px 60px rgba(0,0,0,0.3)' }}>
        ...
        <button
          style={{ width:'100%', padding:10, background:'#1e3a5f', color:'#fff', border:'none', borderRadius:8, fontSize:15, cursor:'pointer', marginBottom:10, opacity:load?0.7:1 }}
          onClick={handleLogin} disabled={load}>
          {load ? 'Entrando...' : 'Entrar'}
        </button>

        <button
          style={{ width:'100%', padding:10, background:'transparent', color:'#1e3a5f', border:'2px solid #1e3a5f', borderRadius:8, fontSize:15, cursor:'pointer', marginBottom:16 }}
          onClick={onCadastro}>
          Criar nova conta
        </button>
