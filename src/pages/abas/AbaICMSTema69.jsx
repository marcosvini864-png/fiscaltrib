...
const LINHAS_GHOST = Array(5).fill(null).map((_, i) => ({
  ...
  vBC: 0, vICMS: 0, vPIS: 0, vCOFINS: 0, credito: 0, temICMS: false, ghost: true,
}))
...
export default function AbaICMSTema69({ cliente, regime }) {
  ...
  async function carregarHistorico() {
    ...
    const { data } = await supabase.from('diagnosticos_icms_tema69').select('*').eq('cliente_id', cliente.id).order('created_at', { ascending: false })
    setHistorico(data || [])
      ...
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ background: S.white, border: `1px solid ${S.border}`, borderRadius: 10, padding: '14px 18px', minWidth: 260 }}>
          ...
          <button onClick={() => inputRef.current?.click()}
            style={{ width: '100%', padding: '8px 0', background: S.blue, color: S.white, border: 'none', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
            ⬆ Selecionar Arquivos
          </button>
        </div>
      </div>
          ...
          <div style={{ background:S.white, borderRadius:10, border:`1px solid ${S.border}`, marginBottom:16, overflow:'hidden' }}>
                  ...
                  {itensPagina.map((item,i) => {
                    ...
                    return (
                      <tr key={i} style={{ borderBottom:`1px solid ${S.border}`, background:isGhost?S.ghost:sel?'#eff6ff':i%2===0?S.white:'#FAFAFA' }}>
                        ...
                        <td style={{ padding:'7px 10px', color:isGhost?S.ghostText:S.text }}>{isGhost?'R$ —,——':fmtR(item.vBC)}</td>
                        <td style={{ padding:'7px 10px', color:isGhost?S.ghostText:item.vICMS>0?S.red:S.muted, fontWeight:isGhost?400:item.vICMS>0?700:400 }}>{isGhost?'R$ —,——':fmtR(item.vICMS)}</td>
                        <td style={{ padding:'7px 10px', color:isGhost?S.ghostText:S.text }}>{isGhost?'R$ —,——':fmtR(item.vPIS)}</td>
                        <td style={{ padding:'7px 10px', color:isGhost?S.ghostText:S.text }}>{isGhost?'R$ —,——':fmtR(item.vCOFINS)}</td>
                        <td style={{ padding:'7px 10px', fontWeight:700, color:isGhost?S.ghostText:item.credito>0?S.green:S.muted }}>{isGhost?'R$ —,——':fmtR(item.credito)}</td>
                        <td style={{ padding:'7px 10px' }}>
                          {isGhost
                            ...
                            : <Badge tipo={item.temICMS?'com_icms':'sem_icms'} />
                          }
                        </td>
            ...
            {!temResultado && (
              <div style={{ padding:'16px 20px', borderTop:`1px solid ${S.border}`, textAlign:'center', fontSize:12, color:S.ghostText }}>
                Importe arquivos XML de NF-e para identificar ICMS na base de calculo do PIS/COFINS
              </div>
            )}
            ...
            <div style={{ padding:'10px 16px', borderTop:`1px solid ${S.border}`, display:'flex', alignItems:'center', justifyContent:'space-between', fontSize:12, color:S.muted, flexWrap:'wrap', gap:8 }}>
              <span>{temResultado ? `${itensFiltrados.length} registros — Pagina ${pagina} de ${totalPaginas}` : 'Aguardando importacao'}</span>
