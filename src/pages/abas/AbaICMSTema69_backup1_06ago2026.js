...
function Badge({ tipo }) {
  const map = {
    com_icms:    { label: 'Com ICMS',      bg: '#fef2f2', color: '#dc2626', border: '#fecaca' },
    sem_icms:    { label: 'Sem ICMS',      bg: '#f1f5f9', color: '#64748B', border: '#cbd5e1' },
...
export default function AbaICMSTema69({ cliente, regime }) {
  ...
  async function salvarDiagnostico() {
    ...
    try {
      ...
      const { error } = await supabase.from('diagnosticos_icms_tema69').insert([{
        usuario_id: user.id,
        ...
        total_com_icms: itens.filter(i => i.temICMS).length,
        vICMS_total: itens.reduce((s, i) => s + i.vICMS, 0),
  ...
  async function excluirDiagnostico(id) {
    ...
    await supabase.from('diagnosticos_icms_tema69').delete().eq('id', id)
  ...
  const itensFiltrados = itens.filter(i => {
    if (filtro === 'com_icms' && !i.temICMS) return false
    if (filtro === 'sem_icms' &&  i.temICMS) return false
    if (busca) {
      const b = busca.toLowerCase()
      return i.emitente.toLowerCase().includes(b) || i.nNF.includes(b) || i.competencia.includes(b)
    }
    return true
  })
    ...
    <div style={{ fontFamily: 'Inter, Arial, sans-serif', color: S.text }} onClick={() => setMenuAberto(null)}>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 13, color: S.muted, marginBottom: 2 }}>
          Motor do Simples / <strong style={{ color: S.text }}>Exclusao ICMS — STF Tema 69</strong>
        </div>
        <div style={{ fontSize: 20, fontWeight: 700, color: S.navy }}>Exclusao ICMS — STF Tema 69</div>
          ...
          <div style={{ background: S.white, borderRadius: 10, border: `1px solid ${S.border}`, marginBottom: 16, overflow: 'hidden' }}>
            ...
            <div style={{ padding: 16 }}>
              <div onDrop={onDrop} onDragOver={e => e.preventDefault()} onClick={() => inputRef.current?.click()}
                ...
                <div style={{ fontSize: 12, color: S.muted, marginBottom: 8 }}>Extensoes aceitas: <strong style={{ color: S.navy }}>.xml .txt .zip .rar .DEC .rec .RE .DIA .prf .pdf</strong></div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    <thead>
                      <tr style={{ background: S.thBg }}>
                        {['Arquivo', 'Tamanho', 'Data de Upload', 'Progresso', 'Status', ''].map(h => (
          ...
          {temResultado && (
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 16 }}>
                {[
                  { label: 'Total de NF-es',         valor: itens.length,         cor: S.navy   },
                  { label: 'NF-es com ICMS',          valor: totalComICMS,          cor: S.red    },
                  { label: 'ICMS Total Identificado', valor: fmtR(vICMSTotal),      cor: S.orange },
                  { label: 'Credito Estimado PIS/COFINS', valor: fmtR(creditoTotal), cor: S.green },
                ].map((k, i) => (
              ...
              <div style={{ background: S.white, borderRadius: 10, border: `1px solid ${S.border}`, marginBottom: 16, overflow: 'hidden' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    <thead>
                      <tr style={{ background: S.thBg }}>
                        ...
                        {['NF', 'Competencia', 'Emitente', 'Base de Calculo', 'ICMS', 'PIS', 'COFINS', 'Credito Estimado', 'Classificacao', 'Acoes'].map(h => (
                      ...
                      {itensPagina.map((item, i) => {
                        ...
                        return (
                          <tr key={i} style={{ borderBottom: `1px solid ${S.border}`, background: sel ? '#eff6ff' : i % 2 === 0 ? S.white : '#FAFAFA' }}>
                            ...
                            <td style={{ padding: '7px 10px' }}>{fmtR(item.vBC)}</td>
                            <td style={{ padding: '7px 10px', color: item.vICMS > 0 ? S.red : S.muted, fontWeight: item.vICMS > 0 ? 700 : 400 }}>{fmtR(item.vICMS)}</td>
                            <td style={{ padding: '7px 10px' }}>{fmtR(item.vPIS)}</td>
                            <td style={{ padding: '7px 10px' }}>{fmtR(item.vCOFINS)}</td>
                            <td style={{ padding: '7px 10px', fontWeight: 700, color: item.credito > 0 ? S.green : S.muted }}>{fmtR(item.credito)}</td>
                            <td style={{ padding: '7px 10px' }}><Badge tipo={item.temICMS ? 'com_icms' : 'sem_icms'} /></td>
                ...
                <div style={{ padding: '10px 16px', borderTop: `1px solid ${S.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12, color: S.muted, flexWrap: 'wrap', gap: 8 }}>
                  <span>{itensFiltrados.length} registros — Pagina {pagina} de {totalPaginas}</span>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
                ...
                <button onClick={novaAnalise} style={{ padding: '9px 16px', background: 'none', border: `1px solid ${S.border}`, borderRadius: 6, fontSize: 13, cursor: 'pointer', color: S.muted }}>
                  Nova analise
                </button>
              </div>
            </>
            ...
            <>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  <thead>
                    <tr style={{ background: S.thBg }}>
                      {['Data', 'Periodo', 'NF-es', 'Com ICMS', 'ICMS Total', 'Credito Estimado', 'Status', 'Acoes'].map(h => (
                        <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: S.thText, fontWeight: 600, fontSize: 11, whiteSpace: 'nowrap' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {historico.map((diag, i) => (
                      <tr key={i} style={{ borderBottom: `1px solid ${S.border}`, background: i % 2 === 0 ? S.white : '#FAFAFA' }}>
                        ...
                        <td style={{ padding: '7px 10px', color: S.red, fontWeight: 700 }}>{diag.total_com_icms}</td>
                        <td style={{ padding: '7px 10px' }}>{fmtR(diag.vICMS_total)}</td>
