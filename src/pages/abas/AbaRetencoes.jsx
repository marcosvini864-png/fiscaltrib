        ...
        if (arq.nome.toLowerCase().endsWith('.xml')) {
          ...
          for (const xml of xmls) {
            try {
              ...
              // Simples Nacional nao deve ter retencao de PIS/COFINS/CSLL (LC 123/2006 art. 3 §4)
              const indevida = temRetencao && regime === 'Simples Nacional'
              todosItens.push({
                nNF: nfe.nNF || '-',
                competencia: nfe.competencia,
                emitente: nfe.emitNome || '-',
                vNF: parseFloat(nfe.vNF || 0),
                vRetPIS,
                vRetCOFINS,
                vRetCSLL,
                vRetencao,
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 16 }}>
                {[
                  ...
                  { label: 'Retencoes Indevidas',       valor: totalIndevidas,    cor: S.red    },
                  { label: 'Total Retencoes Encontradas', valor: fmtR(valorTotal), cor: S.orange },
                  { label: 'Valor Recuperavel',          valor: fmtR(creditoTotal),cor: S.green  },
                ].map((k, i) => (
              ...
              <div style={{ background: S.white, borderRadius: 10, border: `1px solid ${S.border}`, marginBottom: 16, overflow: 'hidden' }}>
                      ...
                      {itensPagina.map((item, i) => {
                        ...
                        return (
                          <tr key={i} style={{ borderBottom: `1px solid ${S.border}`, background: sel?'#eff6ff':i%2===0?S.white:'#FAFAFA' }}>
                            ...
                            <td style={{ padding: '7px 10px' }}>{fmtR(item.vNF)}</td>
                            <td style={{ padding: '7px 10px', color: item.vRetPIS > 0 ? S.red : S.muted }}>{fmtR(item.vRetPIS)}</td>
                            <td style={{ padding: '7px 10px', color: item.vRetCOFINS > 0 ? S.red : S.muted }}>{fmtR(item.vRetCOFINS)}</td>
                            <td style={{ padding: '7px 10px', color: item.vRetCSLL > 0 ? S.red : S.muted }}>{fmtR(item.vRetCSLL)}</td>
                            <td style={{ padding: '7px 10px', fontWeight: 700, color: item.vRetencao > 0 ? S.red : S.muted }}>{fmtR(item.vRetencao)}</td>
                ...
                <div style={{ padding: '10px 16px', borderTop: `1px solid ${S.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12, color: S.muted, flexWrap: 'wrap', gap: 8 }}>
                  <span>{itensFiltrados.length} NF-es — Pagina {pagina} de {totalPaginas}</span>
