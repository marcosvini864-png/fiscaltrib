...
const LINHAS_GHOST = Array(5).fill(null).map((_, i) => ({
  ...
  descricao: 'Descricao do Produto', ncm: '0000.00.00', cfop: '0000', cst: '00',
  vProd: 0, vST: 0, credito: 0, temST: false, ghost: true,
}))
...
export default function AbaICMSST({ cliente, regime }) {
  ...
  async function carregarHistorico() {
    ...
    const { data } = await supabase.from('diagnosticos_icms_st').select('*').eq('cliente_id', cliente.id).order('created_at', { ascending: false })
    setHistorico(data || [])
            ...
            try {
              ...
              ;(nfe.itens || []).forEach(item => {
                ...
                todosItens.push({
                  ...
                  descricao: item.xProd||'-', ncm: item.ncm||'-', cfop: item.CFOP||'-',
                  cst: item.CST||item.CSOSN||'-', vProd: item.vProd||0, vST, temST, credito, arquivo: arq.nome,
                })
                qtd++
              })
            } catch {}
  ...
  const dadosIA = temResultado ? {
    ...
    creditoTotal,
    regime,
    top10: itens.filter(i => i.temST).slice(0, 10).map(i => ({
      nNF: i.nNF, descricao: i.descricao, ncm: i.ncm,
      vST: i.vST, credito: i.credito, competencia: i.competencia,
    }))
  } : null
    ...
    <div style={{ fontFamily: 'Inter, Arial, sans-serif', color: S.text }} onClick={() => setMenuAberto(null)}>
      ...
      {aba === 'importar' && (
        <>
          {/* ANALISADOR IA */}
          <AnalisadorIA
            contexto="Credito ICMS-ST nas Entradas"
            dados={dadosIA}
            cliente={cliente}
            regime={regime}
          />
          ...
          <div style={{ background:S.white, borderRadius:10, border:`1px solid ${S.border}`, marginBottom:16, overflow:'hidden' }}>
                  ...
                  {itensPagina.map((item,i) => {
                    ...
                    return (
                      <tr key={i} style={{ borderBottom:`1px solid ${S.border}`, background:isGhost?S.ghost:sel?'#eff6ff':i%2===0?S.white:'#FAFAFA' }}>
                        ...
                        <td style={{ padding:'7px 10px', maxWidth:130, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', color:isGhost?S.ghostText:S.text }}>{item.emitente}</td>
                        <td style={{ padding:'7px 10px', maxWidth:150, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', color:isGhost?S.ghostText:S.text }}>{item.descricao}</td>
                        <td style={{ padding:'7px 10px', color:isGhost?S.ghostText:S.muted }}>{item.ncm}</td>
                        <td style={{ padding:'7px 10px', color:isGhost?S.ghostText:S.muted }}>{item.cfop}</td>
                        <td style={{ padding:'7px 10px', color:isGhost?S.ghostText:S.muted }}>{item.cst}</td>
                        <td style={{ padding:'7px 10px', color:isGhost?S.ghostText:S.text }}>{isGhost?'R$ —,——':fmtR(item.vProd)}</td>
                        <td style={{ padding:'7px 10px', color:isGhost?S.ghostText:item.vST>0?S.orange:S.muted, fontWeight:isGhost?400:item.vST>0?700:400 }}>{isGhost?'R$ —,——':fmtR(item.vST)}</td>
                        ...
                        <td style={{ padding:'7px 10px' }}>
                          {isGhost
                            ...
                            : <Badge tipo={item.temST?'com_st':'sem_st'} />
                          }
                        </td>
            ...
            {!temResultado && (
              <div style={{ padding:'16px 20px', borderTop:`1px solid ${S.border}`, textAlign:'center', fontSize:12, color:S.ghostText }}>
                Importe arquivos XML de NF-e para identificar ICMS-ST nas entradas
              </div>
            )}
            ...
            <div style={{ padding:'10px 16px', borderTop:`1px solid ${S.border}`, display:'flex', alignItems:'center', justifyContent:'space-between', fontSize:12, color:S.muted, flexWrap:'wrap', gap:8 }}>
              <span>{temResultado ? `${itensFiltrados.length} itens — Pagina ${pagina} de ${totalPaginas}` : 'Aguardando importacao'}</span>
