...
export default function PrazosPrescricionais({ active }) {
  ...
  async function carregarEntradas() {
    ...
    const { data } = await supabase
      .from('entradas')
      .select('*')
      .eq('cliente_id', active.id)
    setEntradas(data || [])
    setLoading(false)
  }

  // Só entradas com crédito > 0
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ background:'linear-gradient(135deg, #0B1F4D 0%, #163B8C 100%)', borderRadius:16, padding:'28px 32px', marginBottom:24, color:'#fff' }}>
        ...
        <h2 style={{ fontSize:24, fontWeight:900, marginBottom:8 }}>⏳ Controle Prescricional</h2>
        <div style={{ fontSize:14, color:'#93c5fd' }}>
          {active?.razao_social} · Prazo de 5 anos contados do pagamento indevido (art. 168 CTN)
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:12, marginBottom:24 }}>
        {[
          ...
          <div key={i} onClick={() => setFiltro(i===0?'prescritos':i===1||i===2?'criticos':i===3?'atencao':'confortavel')}
            style={{ background:k.bg, border:`2px solid ${k.borda}`, borderRadius:12, padding:'16px 12px', textAlign:'center', cursor:'pointer', transition:'transform 0.15s' }}
      ...
      {ordenadas.length === 0 ? (
        <div style={{ background:'#fff', borderRadius:12, border:'2px solid #e2e8f0', padding:48, textAlign:'center', color:'#94a3b8' }}>
          ...
          <div style={{ fontSize:13, marginTop:8 }}>Adicione entradas com crédito para monitorar os prazos prescricionais</div>
        </div>
      ) : (
                  ...
                  <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:16 }}>
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
                        ...
                        <span style={{ fontSize:13, fontWeight:700, color:'#0B1F4D' }}>
                          {fmtComp(e.competencia)} — {e.tributo}
                        </span>
                      </div>

                      {e.tipo_oportunidade && (
                        <div style={{ fontSize:12, color:'#64748b', marginBottom:8 }}>{e.tipo_oportunidade}</div>
                      )}

                      {/* Linha do tempo */}
                      <div style={{ display:'flex', gap:20, fontSize:12, flexWrap:'wrap' }}>
                        <div>
                          <span style={{ color:'#94a3b8', fontWeight:600 }}>Competência: </span>
                          <span style={{ color:'#1e293b', fontWeight:700 }}>{fmtComp(e.competencia)}</span>
                        </div>
                        <div>
                          <span style={{ color:'#94a3b8', fontWeight:600 }}>Prescreve em: </span>
                          <span style={{ color:sem.cor, fontWeight:800 }}>
                            {dias <= 0 ? 'PRESCRITO' : `${dias} dias`}
                          </span>
                        </div>
                        <div>
                          <span style={{ color:'#94a3b8', fontWeight:600 }}>Data limite: </span>
                          <span style={{ color:'#1e293b', fontWeight:700 }}>{limite.toLocaleDateString('pt-BR')}</span>
                        </div>
                      </div>

                      {/* Alerta especial */}
                      {dias > 0 && dias <= 90 && (
                        <div style={{ marginTop:10, background:'#dc2626', borderRadius:8, padding:'8px 12px', fontSize:12, color:'#fff', fontWeight:700 }}>
                          ⚡ AÇÃO IMEDIATA: Menos de 90 dias para prescrição! Inicie o processo de recuperação agora.
                        </div>
                      )}
                      {dias > 90 && dias <= 180 && (
                        <div style={{ marginTop:10, background:'#ea580c', borderRadius:8, padding:'8px 12px', fontSize:12, color:'#fff', fontWeight:600 }}>
                          ⚠️ CRÍTICO: Prazo inferior a 6 meses. Priorize este crédito.
                        </div>
                      )}
                      {dias <= 0 && (
                        <div style={{ marginTop:10, background:'#7f1d1d', borderRadius:8, padding:'8px 12px', fontSize:12, color:'#fff', fontWeight:700 }}>
                          💀 PRESCRITO em {limite.toLocaleDateString('pt-BR')} — crédito não pode mais ser recuperado por esta via.
                        </div>
                      )}
                    </div>
