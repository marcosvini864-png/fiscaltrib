...
function parseXMLNFe(xmlStr) {
  ...
  const cnpjEmit = get('emit CNPJ') || get('CNPJ')
  const xNome    = get('emit xNome') || get('xNome') || 'Não identificado'
  const chNFe    = get('chNFe') || doc.querySelector('[Id]')?.getAttribute('Id')?.replace('NFe', '') || ''
  const nNF      = get('nNF') || ''
  const cfop     = get('CFOP') || ''

  return { competencia, vNF, vICMS, vPIS, vCOFINS, vISS, cnpjEmit, xNome, chNFe, nNF, cfop, valido: !!competencia && vNF > 0 }
}
...
function parsePGDAS(xmlStr) {
  ...
  // Tenta estrutura padrão do PGDAS-D
  const competencia = doc.querySelector('periodoApuracao')?.textContent ||
                      doc.querySelector('competencia')?.textContent || ''
  const receitaBruta = parseFloat(
    doc.querySelector('receitaBrutaTotal')?.textContent ||
    doc.querySelector('totalReceita')?.textContent || 0
  )
  const dasDevido = parseFloat(
    doc.querySelector('valorDevido')?.textContent ||
    doc.querySelector('dasDevido')?.textContent || 0
  )
  ...
  return {
    ...
    receitaBruta, dasDevido, cnpj, razaoSocial, anexo, aliquota,
    valido: !!competencia && receitaBruta > 0,
  }
}
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '0 16px 40px' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0f2444 0%, #1e3a5f 50%, #1a4f7a 100%)', borderRadius: '0 0 24px 24px', padding: '36px 40px 40px', marginBottom: 32, color: '#fff', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <p style={{ fontSize: 16, color: '#9db8d8', marginBottom: 28, lineHeight: 1.6, maxWidth: 520 }}>
            Importe XMLs, PDFs e arquivos fiscais e deixe o sistema preencher os dados automaticamente. Economia de horas por semana.
          </p>
      ...
      <div style={{ background: '#fff', borderRadius: 14, border: '2px solid #e2e8f0', padding: '20px 24px', marginBottom: 24 }}>
        <label style={{ fontSize: 14, fontWeight: 700, color: '#1e3a5f', display: 'block', marginBottom: 10 }}>
          👤 Cliente para importação:
        </label>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
        {ABAS.map(a => (
          <button key={a.id} onClick={() => a.status === 'ativo' && setAba(a.id)}
            style={{
              padding: '10px 18px', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: a.status === 'ativo' ? 'pointer' : 'default',
      ...
      {!clienteId ? (
        ...
        <AbaDAS clienteId={clienteId} cliente={cliente} />
      ) : (
...
function AbaXMLNFe({ clienteId, cliente }) {
  ...
  function processarArquivos(files) {
    ...
    Array.from(files).forEach(f => {
      ...
      reader.onload = e => {
        const resultado = parseXMLNFe(e.target.result)
        if (resultado.valido) novos.push({ ...resultado, arquivo: f.name })
        else novosErros.push(`${f.name}: XML inválido ou NF-e não reconhecida`)
        pendentes--
    ...
    <div>
      ...
      <div
        ...
        style={{
          ...
          border: `3px dashed ${dragOver ? '#3b82f6' : '#e2e8f0'}`,
      ...
      >
        ...
        <div style={{ display: 'inline-block', padding: '10px 24px', background: '#1e3a5f', color: '#fff', borderRadius: 8, fontSize: 14, fontWeight: 700 }}>
          📂 Selecionar XMLs
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 24 }}>
            {[
              ...
              { label: 'Faturamento total',   valor: fmtR(nfes.reduce((s,n) => s+n.vNF,0)),                   cor: '#16a34a' },
              { label: 'ICMS total',          valor: fmtR(nfes.reduce((s,n) => s+n.vICMS,0)),                 cor: '#d97706' },
              { label: 'PIS+COFINS total',    valor: fmtR(nfes.reduce((s,n) => s+n.vPIS+n.vCOFINS,0)),       cor: '#7c3aed' },
            ].map((c, i) => (
          ...
          {salvo ? (
            <div style={{ background: '#f0fdf4', border: '2px solid #86efac', borderRadius: 12, padding: '16px 24px', textAlign: 'center', fontSize: 15, fontWeight: 700, color: '#16a34a' }}>
              ✅ Dados salvos com sucesso nas entradas do cliente!
            </div>
          ) : (
...
function AbaPGDAS({ clienteId, cliente }) {
  ...
  function processarArquivo(file) {
    ...
    const reader = new FileReader()
    reader.onload = e => {
      const resultado = parsePGDAS(e.target.result)
      if (resultado.valido) setDados(resultado)
      else setErro('Arquivo XML não reconhecido como PGDAS-D. Verifique o arquivo.')
    }
  ...
  return (
    <div>
      ...
      >
        ...
        <div style={{ display: 'inline-block', padding: '10px 24px', background: '#1e3a5f', color: '#fff', borderRadius: 8, fontSize: 14, fontWeight: 700 }}>
          📂 Selecionar XML do PGDAS-D
        </div>
        <input ref={inputRef} type="file" accept=".xml" style={{ display: 'none' }} onChange={e => processarArquivo(e.target.files[0])} />
      </div>

      {erro && (
        <div style={{ background: '#fff1f2', border: '1px solid #fecdd3', borderRadius: 12, padding: '14px 20px', marginBottom: 20, color: '#dc2626', fontSize: 14, fontWeight: 600 }}>
          ⚠️ {erro}
        </div>
      )}
      ...
      {dados && (
        <div>
          ...
          {salvo ? (
            <div style={{ background: '#f0fdf4', border: '2px solid #86efac', borderRadius: 12, padding: '16px 24px', textAlign: 'center', fontSize: 15, fontWeight: 700, color: '#16a34a' }}>
              ✅ Dados do PGDAS-D salvos com sucesso!
            </div>
          ) : (
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={() => { setDados(null); setSalvo(false) }} style={{ padding: '12px 24px', background: '#f8fafc', color: '#64748b', border: '2px solid #e2e8f0', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
                🗑️ Limpar
              </button>
...
function AbaDAS({ clienteId, cliente }) {
  const [linhas,   setLinhas]   = useState([{ competencia: '', valor: '', situacao: 'pago' }])
  const [salvando, setSalvando] = useState(false)
  const [salvo,    setSalvo]    = useState(false)

  function addLinha() { setLinhas(p => [...p, { competencia: '', valor: '', situacao: 'pago' }]) }
  function updLinha(i, k, v) { setLinhas(p => p.map((l, j) => j === i ? { ...l, [k]: v } : l)) }
