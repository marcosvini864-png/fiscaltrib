...
function gerarPDF({ cliente, oportunidades, score, planoAcao }) {
  ...
  doc.text(`Período analisado: ${cliente?.competencia_inicio || '—'} a ${cliente?.competencia_fim || '—'}`, M+6, y+37)
  y += 48
  ...
  doc.text(`${oportunidades.length} oportunidade(s) tributária(s) identificada(s)`, M + 55, y + 37)
  doc.text(`Potencial total estimado: ${fmtR(totalPotencial)}`, M + 55, y + 44)
  y += 60
  ...
  // ══════════════════════════════════════════════════════════════════
  // PÁGINA 2 — OPORTUNIDADES ENCONTRADAS
  // ══════════════════════════════════════════════════════════════════
  doc.addPage()
  ...
  autoTable(doc, {
    ...
    foot: [['TOTAL GERAL', '', fmtR(oportunidades.reduce((s,o)=>s+(o.mediaMensal||o.potencial/60)*12,0)), fmtR(oportunidades.reduce((s,o)=>s+(o.mediaMensal||o.potencial/60)*36,0)), fmtR(totalPotencial)]],
  ...
  doc.setFontSize(28); doc.setFont('helvetica','bold'); doc.setTextColor(34,197,94)
  ...
  doc.text(`${oportunidades.length} teses identificadas · Regime ${cliente?.regime || ''}`, W/2, y+42, { align:'center' })
  doc.text('Estimativa baseada nos dados fiscais analisados', W/2, y+50, { align:'center' })
  y += 65
  ...
  periodos.forEach((p, i) => {
    ...
    doc.setFillColor(i === 3 ? ...NAVY : ...LIGHT)
    doc.roundedRect(x, y, pw, 32, 3, 3, 'F')
    doc.setFontSize(8); doc.setFont('helvetica','bold')
    doc.setTextColor(i === 3 ? 124 : ...GRAY, i === 3 ? 196 : ...GRAY, i === 3 ? 255 : ...GRAY)
    doc.text(`${p} meses`, x + pw/2, y + 9, { align:'center' })
    doc.setFontSize(10); doc.setFont('helvetica','bold')
    doc.setTextColor(i === 3 ? 34 : ...NAVY, i === 3 ? 197 : ...NAVY, i === 3 ? 94 : ...NAVY)
    doc.text(fmtR(val), x + pw/2, y + 19, { align:'center' })
    doc.setFontSize(7); doc.setFont('helvetica','normal')
    doc.setTextColor(i === 3 ? 124 : ...GRAY, i === 3 ? 196 : ...GRAY, i === 3 ? 255 : ...GRAY)
    doc.text(`${Math.round(pct*100)}% do total`, x + pw/2, y + 27, { align:'center' })
  })
  y += 42
  ...
  autoTable(doc, {
    ...
    foot: [['TOTAL', fmtR(oportunidades.reduce((s,o)=>s+(o.mediaMensal||o.potencial/60),0)),
  ...
  // Caixa de encerramento comercial
  doc.setFillColor(...NAVY)
  ...
  // Salvar
  const nomeArq = `FiscalTrib_Diagnostico_${(cliente?.razao_social||'cliente').replace(/[^a-zA-Z0-9]/g,'_')}_${new Date().toISOString().slice(0,10)}.pdf`
  doc.save(nomeArq)
}
...
export default function Relatorio({ active, ents }) {
  ...
  // Monta oportunidades unificando entradas manuais + recuperações
  function montarOportunidades() {
    const ops = []

    // Das entradas manuais
    const entsCred = (ents || []).filter(e => e.credito > 0)
    entsCred.forEach(e => {
      ops.push({
        tese: e.tipo_oportunidade || e.tributo || 'Crédito identificado',
        descricao: `Competência ${e.competencia} · ${e.tributo}`,
        mediaMensal: e.credito,
        potencial: e.credito * 60,
        risco: e.risco || 'medio',
        ncms: [],
      })
    })
    ...
    recuperacoes.forEach(r => {
      if (!ops.find(o => o.tese === r.tese_aplicada)) {
        ops.push({
          tese: r.tese_aplicada || 'Recuperação em andamento',
          descricao: `Processo iniciado · Status: ${r.status}`,
          mediaMensal: (r.potencial_recuperavel || 0) / 60,
          potencial: r.potencial_recuperavel || 0,
          risco: r.risco || 'medio',
          ncms: [],
        })
      }
  ...
  function montarPlanoAcao(oportunidades) {
    return oportunidades
      .sort((a, b) => {
        const ord = { baixo: 0, medio: 1, alto: 2 }
        return ord[a.risco] - ord[b.risco]
      })
      .map((op, i) => ({
        titulo: i === 0 ? `Recuperar: ${op.tese}` : i === 1 ? `Revisar: ${op.tese}` : `Ajustar: ${op.tese}`,
        descricao: op.descricao,
        potencial: op.potencial,
        risco: op.risco,
      }))
  }

  function calcularScore(oportunidades) {
    return Math.min(100, Math.max(20,
      100
      - oportunidades.filter(o => o.risco === 'baixo').length * 12
      - oportunidades.filter(o => o.risco === 'medio').length * 6
    ))
  }
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0B1F4D 0%, #163B8C 100%)', borderRadius: 16, padding: '28px 32px', marginBottom: 24, color: '#fff' }}>
        ...
        <h2 style={{ fontSize: 24, fontWeight: 900, marginBottom: 8 }}>📄 Relatório Matador</h2>
        <p style={{ fontSize: 14, color: '#93c5fd', marginBottom: 0 }}>
          PDF comercial de 4 páginas · Leve para o cliente e feche o contrato
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
          ...
          <div style={{ padding: '16px 20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: '#64748b' }}>{oportunidades.length} oportunidade(s) identificada(s)</div>
              </div>
            </div>
            <div style={{ fontSize: 12, color: '#64748b' }}>
              <strong style={{ color: '#0B1F4D' }}>{active?.razao_social}</strong><br />
              {active?.cnpj} · {active?.regime}
            </div>
          </div>
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '2px solid #e2e8f0', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '16px 20px' }}>
            {oportunidades.length === 0 ? (
              <div style={{ fontSize: 13, color: '#94a3b8', textAlign: 'center', padding: '12px 0' }}>Nenhuma oportunidade identificada ainda</div>
            ) : (
              oportunidades.slice(0, 4).map((op, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid #f1f5f9', fontSize: 13 }}>
                  <span style={{ color: '#1e293b', fontWeight: 500 }}>
                    {op.risco === 'baixo' ? '🟢' : '🟡'} {op.tese?.slice(0, 30)}{op.tese?.length > 30 ? '...' : ''}
                  </span>
                  <span style={{ fontWeight: 700, color: '#16a34a' }}>{fmtR(op.potencial)}</span>
                </div>
              ))
            )}
            {oportunidades.length > 4 && <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 6 }}>+{oportunidades.length - 4} mais no PDF...</div>}
          </div>
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '2px solid #e2e8f0', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '16px 20px', textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 4 }}>POTENCIAL ESTIMADO (60 MESES)</div>
      ...
      <button
        onClick={handleGerar}
        disabled={gerando || oportunidades.length === 0}
      ...
      >
        {gerando ? '⏳ Gerando PDF...' : oportunidades.length > 0 ? `📄 Baixar Relatório Matador — ${fmtR(totalPotencial)}` : '⚠️ Importe dados ou adicione entradas para gerar o relatório'}
      </button>
      ...
      {oportunidades.length === 0 && (
        <div style={{ background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: 10, padding: '14px 18px', fontSize: 13, color: '#92400e', textAlign: 'center' }}>
          💡 Para gerar o relatório, importe XMLs de NF-e ou adicione entradas de dados manualmente no módulo de <strong>Entrada de Dados</strong>.
        </div>
      )}
    </div>
  )
