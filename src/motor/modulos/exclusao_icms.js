...
export async function analisarExclusaoICMS(nfes, cliente, opcoes = {}, BaseTributaria) {
  ...
  try {
	...
	console.log('EXCLUSAO_ICMS primeira nfe:', JSON.stringify({ tpNF: nfes[0]?.tpNF, vICMS: nfes[0]?.vICMS, dhEmi: nfes[0]?.dhEmi, dEmi: nfes[0]?.dEmi, competencia: nfes[0]?.competencia }))
    ...
    nfesSaida.forEach(nfe => {
      ...
      const item = {
        ...
        vProd: nfe.vNF, vItemICMS: nfe.vICMS, vItemPIS: nfe.vPIS || 0,
        vItemCOFINS: nfe.vCOFINS || 0, vItemST: nfe.vST || 0, vItemIPI: nfe.vIPI || 0,
    ...
    const oportunidade = {
      ...
      descricao:     `${diagnostico.totalNFesComICMS} NF-e(s) com ICMS destacado em ${consolidado.totalCompetencias} competência(s). ICMS total excluível: R$ ${consolidado.totalICMSExcluido.toFixed(2)}. Crédito PIS (${aliq.pis}%) + COFINS (${aliq.cofins}%) sobre o ICMS excluído.`,
      score:         { valor: scoreValor, classificacao: scoreValor >= 80 ? 'Excelente' : 'Bom', componentes: [] },
      ...
      fundamentacao: {
        ...
        observacao: 'ICMS destacado na NF-e — conforme entendimento do STF.',
      },
      calculos: {
        ...
        memoriaCalculo: [
          `1. ${diagnostico.totalNFesComICMS} NF-e(s) com ICMS destacado identificadas`,
          `2. Total ICMS: R$ ${consolidado.totalICMSExcluido.toFixed(2)}`,
      ...
      recomendacao: {
        titulo:     'Ingressar com PER/DCOMP ou ação judicial',
        prioridade: 'ALTA',
        via:        'JUDICIAL',
        passos: [
          { ordem: 1, acao: 'Levantar todas as NF-es de saída do período retroativo', prazo: '30 dias' },
          { ordem: 2, acao: 'Apurar o ICMS destacado por competência', prazo: '15 dias' },
          { ordem: 3, acao: 'Calcular crédito de PIS/COFINS sobre o ICMS excluído', prazo: '15 dias' },
    ...
    resultado.score = {
      valor:         scoreValor,
      classificacao: scoreValor >= 80 ? 'Excelente' : 'Bom',
      componentes:   [],
    ...
    if (consolidado.periodo?.modulacaoAplicada) {
      resultado.riscos.push({
        ...
        nivel:      'BAIXO',
        mitigacao:  'Verificar se havia ação ou pedido anterior a 15/03/2017',
      })
    }
    ...
    resultado.relatorio = {
      resumoExecutivo:     `Identificada oportunidade pela exclusão do ICMS da base de PIS/COFINS (Tema 69 STF) para ${cliente.razao_social}. ICMS excluível: R$ ${consolidado.totalICMSExcluido.toFixed(2)}. Crédito estimado: R$ ${consolidado.creditoTotal.toFixed(2)} (60 meses: R$ ${consolidado.creditoPor60Meses.toFixed(2)}).`,
      objetivoAnalise:     `Identificar créditos de PIS/COFINS pela exclusão do ICMS da base de cálculo (RE 574.706 STF — Tema 69) para empresa no regime ${cliente.regime}.`,
      escopoAnalise:       `${diagnostico.totalNFesAnalisadas} NF-e(s), período ${diagnostico.periodoInicio} a ${diagnostico.periodoFim}, ${diagnostico.totalCompetencias} competência(s).`,
      diagnosticoTecnico:  `${diagnostico.totalNFesComICMS} NF-e(s) com ICMS destacado. Total: R$ ${diagnostico.totalICMSIdentificado.toFixed(2)} (${diagnostico.percentualICMS} do faturamento).`,
      oportunidadesTexto:  `Crédito estimado R$ ${consolidado.creditoMensalMedio.toFixed(2)}/mês, potencial 60 meses: R$ ${consolidado.creditoPor60Meses.toFixed(2)}.`,
      riscosTexto:         resultado.riscos.map(r => `[${r.nivel}] ${r.descricao}`).join('. ') || 'Nenhum risco relevante.',
      fundamentacaoTexto:  'RE 574.706 STF (Tema 69): o ICMS não compõe a base de PIS/COFINS. Embargos julgados em 13/05/2021.',
      conclusaoExecutiva:  `Recomenda-se PER/DCOMP ou ação judicial para recuperação de R$ ${consolidado.creditoTotal.toFixed(2)} em créditos de PIS/COFINS (Tema 69 STF). Grau de confiança: ${confianca.grau}.`,
